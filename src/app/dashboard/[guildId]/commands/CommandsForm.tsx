...
export default function CommandsForm({ config, channels, saveAction }: Props) {
  ...
  const [asEmbed, setAsEmbed] = useState(config.blockedCommandAsEmbed);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set('blockedCommandAsEmbed', asEmbed ? 'true' : 'false');
    saveAction(formData);
  };
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            >
              ...
              {channels
                .filter((c) => !blockedChannels.includes(c.id))
                .map((c) => (
          ...
          {notify && (
            <div style={{
              ...
              <div style={{
                ...
                <div style={{
                  width: '40px', height: '24px', borderRadius: '12px',
                  background: asEmbed ? '#C100FF' : '#2b2d31',
                  border: 'none', cursor: 'pointer', position: 'relative',
                  flexShrink: 0, marginTop: '2px',
                  transition: 'background 0.2s',
                }}
                  onClick={() => setAsEmbed(!asEmbed)}
                >
                  <div style={{
                    position: 'absolute', width: '18px', height: '18px',
                    borderRadius: '50%', background: 'white', top: '3px',
                    left: asEmbed ? '19px' : '3px', transition: 'left 0.2s',
                    boxShadow: '0 1px 3px rgba(0,0,0,0.3)',
                  }} />
                </div>
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: '#72767d' }}>
                    A mensagem sera enviada dentro de um embed roxo da ByteUP. Se desativado, sera enviada como texto puro.
                  </div>
                </div>
              </div>
              ...
              {/* Preview do embed */}
              <div style={{
                background: asEmbed ? '#1e2025' : 'transparent',
                borderRadius: '8px',
                padding: asEmbed ? '12px 16px' : '0',
                borderLeft: asEmbed ? '4px solid #C100FF' : 'none',
                fontSize: '13px',
                lineHeight: '1.5',
              }}>
