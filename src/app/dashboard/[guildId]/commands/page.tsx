...
async function saveCommandsAction(guildId: string, formData: FormData) {
  "use server";
  const blockedCommandChannels =
    (formData.get("blockedCommandChannels") as string) || "[]";
  const blockedCommandMessage =
    (formData.get("blockedCommandMessage") as string) ||
    "Voce nao pode executar comandos neste canal!";
  const blockedCommandNotify = formData.get("blockedCommandNotify") === "true";
  const blockedCommandAsEmbed = formData.get("blockedCommandAsEmbed") === "true";  // ← ADICIONA
  ...
  try {
    ...
    if (existing) {
      await prisma.guildConfig.update({
        where: { guildId },
        data: {
          blockedCommandChannels,
          blockedCommandMessage,
          blockedCommandNotify,
          blockedCommandAsEmbed,   // ← ADICIONA
        },
      });
    } else {
...
export default async function CommandsPage({ params }: Props) {
  ...
  config={{
    ...
    blockedCommandMessage:
      config.blockedCommandMessage ||
      "Voce nao pode executar comandos neste canal!",
    blockedCommandNotify: config.blockedCommandNotify ?? true,
    blockedCommandAsEmbed: config.blockedCommandAsEmbed ?? false, 
  }}
