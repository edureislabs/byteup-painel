...
export default function KissForm({ config, saveAction }: Props) {
  const [enabled, setEnabled] = useState(config.kissEnabled ?? true);
  const [title, setTitle] = useState(config.kissTitle || 'Beijo');
  const [color, setColor] = useState(config.kissColor || '#F472B6');
  const [message, setMessage] = useState(config.kissMessage || '{user} deu um beijo em {target}');
  const [imageUrl, setImageUrl] = useState(config.kissImageUrl || 'https://i.pinimg.com/originals/37/df/fe/37dffeecf1a6162e5c6526bfa65b1ca1.gif');
  const [thumbnail, setThumbnail] = useState(config.kissThumbnail || '');
  const [footer, setFooter] = useState(config.kissFooter || '');
  const [timestamp, setTimestamp] = useState(config.kissTimestamp ?? true);
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState(config.kissImageUrl || 'https://i.pinimg.com/originals/37/df/fe/37dffeecf1a6162e5c6526bfa65b1ca1.gif');
  const [thumbnailFile, setThumbnailFile] = useState<File | null>(null);
  const [thumbnailPreview, setThumbnailPreview] = useState(config.kissThumbnail || '');
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set('kissEnabled', enabled ? 'true' : 'false');
    formData.set('kissTitle', title);
    formData.set('kissColor', color);
    formData.set('kissMessage', message);
    formData.set('kissTimestamp', timestamp ? 'true' : 'false');
    formData.set('kissFooter', footer);
    formData.set('kissThumbnail', thumbnailPreview);
    if (file) {
      const reader = new FileReader();
      reader.onload = async () => {
        formData.set('kissImageUrl', reader.result as string);
        await saveAction(formData);
      };
      reader.readAsDataURL(file);
    } else {
      formData.set('kissImageUrl', imageUrl);
      await saveAction(formData);
    }
  };
  ...
  return (
    <div style={{ fontFamily: 'DM Sans, sans-serif', maxWidth: '700px', color: '#dbdee1' }}>
      <h2 style={{ color: '#f2f3f5', marginBottom: '24px' }}>Configuracao do Beijo</h2>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ color: '#dbdee1' }}>Ativar comando de beijo</span>
        ...
        <div style={{ opacity: enabled ? 1 : 0.4, display: 'flex', flexDirection: 'column', gap: '20px', pointerEvents: enabled ? 'auto' : 'none' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '11px', color: '#72767d', marginTop: '4px' }}>
              Use {'{user}'} para quem da o beijo e {'{target}'} para quem recebe.
            </p>
          </div>
          <div>
            <label className="field-label">Imagem/GIF do Beijo</label>
